...
def buildAdversarialGA():
	...
	def fitness_function(a):
		...
		for i in range(len(a)//batch_size):
			retval = np.append(retval, dfire(a[i*batch_size: (i+1) * batch_size, :], [digit_to_generate]*batch_size)[:,1])
		return retval
	current_generated_categories_shared.set_value([digit_to_generate]*NUM_TRAINING_EXAMPLES)


	initial_pop = np.uint8(training_data_shared.get_value()[:100].reshape(100, 28*28))

	gmodel = ga(fitness_function, 28*28, rng, NUM_TRAINING_EXAMPLES*1.1, diversity_limit=0.0001, initial_pop=initial_pop)
	...
	for epoch in range(1, num_epochs):
		...
		if epoch %10 == 0:
			...
			plt.draw()
		data = gmodel.sample_pop(NUM_TRAINING_EXAMPLES)
		...
		while gmodel.median_fitness < 0.5:
			#mutation_rate/=2
			print "Median fitness: ", gmodel.median_fitness
			gmodel.create_next_generation(mutation_rate=mutation_rate)
		gmodel.diversity_limit*=1.5
